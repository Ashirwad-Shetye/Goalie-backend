...
const getGoalStats = asyncHandler(async (req, res) => {
  const pendingGoals = await Goal.find({
    user: req.user.id,
    lastDate: { $gt: Date.parse(new Date()) },
  });
  const finishedGoals = await Goal.find({
    user: req.user.id,
    lastDate: { $lt: Date.parse(new Date()) },
  });
...
const setGoal = asyncHandler(async (req, res) => {
  ...
  sendEmail(
    req.user,
    GOAL_ADDED(req.user.name, req.body.text, req.body.lastDate)
  );
